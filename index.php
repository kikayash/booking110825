...
<div class="modal fade" id="dayDetailModal" tabindex="-1" aria-labelledby="dayDetailModalLabel" aria-hidden="true">
    ...
    try {
        ...
        if (eventModal) {
            ...
            console.log('✅ Event detail modal initialized');
        }
        
        if (dayModal) {
            window.dayDetailModal = new bootstrap.Modal(dayModal);
            console.log('✅ Day detail modal initialized');
        }
...
function showEventDetail(bookingId) {
    console.log('🔍 showEventDetail called with ID:', bookingId, 'Type:', typeof bookingId);
...
function showDayDetail(date) {
    console.log('📅 Loading day bookings for:', date);
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (dosenBookingForm) {
        dosenBookingForm.addEventListener('submit', function(e) {
            ...
            console.log('🚀 Submitting dosen booking...');
            ...
            if (!mataKuliah || !jamMulai || !jamSelesai || !tanggal || !roomId) {
                showBookingAlert('❌ Harap isi semua field yang wajib!', 'danger');
                return;
            }
            
            // Validate time
            if (jamMulai >= jamSelesai) {
                showBookingAlert('❌ Jam selesai harus lebih dari jam mulai!', 'danger');
                return;
            }
            ...
            console.log('📡 Sending booking request to process-booking-iris.php...');
            ...
            .then(data => {
                ...
                if (data.success) {
                    ...
                    let successMsg = `🎉 BOOKING BERHASIL!\n\n`;
                    successMsg += `${data.message}\n\n`;
                    
                    if (data.details) {
                        successMsg += `📋 DETAIL BOOKING:\n`;
                        successMsg += `• ID Booking: #${data.booking_id}\n`;
                        successMsg += `• Ruangan: ${data.details.ruangan}\n`;
                        successMsg += `• Gedung: ${data.details.gedung}\n`;
                        successMsg += `• Tanggal: ${data.details.tanggal}\n`;
                        successMsg += `• Waktu: ${data.details.waktu}\n`;
                        successMsg += `• Mata Kuliah: ${data.details.mata_kuliah}\n`;
                        successMsg += `• Kelas: ${data.details.kelas}\n`;
                        successMsg += `• Dosen: ${data.details.dosen}\n`;
                        successMsg += `• Status: ✅ DISETUJUI\n\n`;
                        successMsg += `💡 CATATAN:\n`;
                        successMsg += `• DIMOHON MENGHUBUNGI CS UNTUK KONFIRMASI\n`;
                    }
                    ...
                    setTimeout(() => {
                        ...
                        showPageAlert(`✅ Booking berhasil! ID: #${data.booking_id} - ${data.details.mata_kuliah}`, 'success');
                    ...
                    if (data.message.includes('bentrok') || data.message.includes('conflict')) {
                        ...
                        errorMsg += '\n• Gunakan NIK dan password yang benar';
                    } else {
            ...
            .catch(error => {
                ...
                errorMsg += '📞 KONTAK DARURAT:\n';
                errorMsg += '• Admin: admin@stie-mce.ac.id\n';
                errorMsg += '• CS: cs@stie-mce.ac.id';
    ...
    .then(data => {
        ...
        if (data.success) {
            ...
            if (data.user) {
                ...
                console.log('👤 User data stored and profile updated:', dosenData);
            }
